...
test.describe('CLI Gradual Integration', () => {
  ...
  test('Level 12: CLI Check Endpoint Security', async ({ request }) => {
    ...
    const validRes = await request.get(
      `${baseURL}/api/cli/${projectId}/${keyId}/check`,
      {
    ...
    // 3. FAIL: Wrong secret
    const invalidSecretRes = await request.get(
      `${baseURL}/api/cli/${projectId}/${keyId}/check`,
      {
    ...
    const project2Res = await request.post(
      `${baseURL}/api/session/project/create`,
    ...
    const wrongProjectRes = await request.get(
      `${baseURL}/api/cli/${projectId2}/${keyId}/check`,
      {
        headers: { 'x-cli-secret': secret },
      },
    );
    ...
    // 5. FAIL: Wrong key ID
    const wrongKeyRes = await request.get(
      `${baseURL}/api/cli/${projectId}/bad-key-id/check`,
      {
        headers: { 'x-cli-secret': secret },
      },
    );
    ...
    const missingHeaderRes = await request.get(
      `${baseURL}/api/cli/${projectId}/${keyId}/check`,
    );
